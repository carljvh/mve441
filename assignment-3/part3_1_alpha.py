...
alpha = np.array([0.09, 0.21, 0.33, 0.45, 0.57, 0.69])

counts = np.zeros((len(alpha),n_features))
betas = np.zeros_like(counts)
for i, a in enumerate(alpha):
    X, y, beta = simulate_data(n=n_samples[2], p=n_features, rng=np.random.default_rng(seed=42), sparsity=sparsity[1], SNR=SNR[2])
    df = pd.DataFrame(X)
    df['y'] = y
    betas[i] = beta
    ...
    for m in range(M):
        ...
        lasso = Lasso(alpha=a).fit(X_bs, y_bs)
...
fig.suptitle('Counts for features with SNR=%s, n=%s, p=%s and s=%s varied over alpha' % (SNR[2], n_samples[2], n_features, sparsity[1]))
for i, ax in enumerate(axes.flatten()):
    ...
    title = "alpha=%s, TPR=%s" % (alpha[i], TPR[i])
    if i == 2: 
        title = "alpha_min=%s, TPR=%s" % (alpha[i], TPR[i])
    if i == 4:
        title="alpha_1se=%s, TPR=%s" % (alpha[i], TPR[i])
    ax.set(title=title, xlabel="features", ylabel="counts")

fig.subplots_adjust(hspace=0.3)
plt.savefig("assignment-3/images/alpha_90.png")
plt.show()