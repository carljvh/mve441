...
counts = np.zeros((len(sparsity),n_features))
betas = np.zeros_like(counts)
for i, s in enumerate(sparsity):
    X, y, beta = simulate_data(n=n_samples[2], p=n_features, rng=np.random.default_rng(seed=42), sparsity=s, SNR=SNR[2])
    df = pd.DataFrame(X)
    df['y'] = y
    betas[i] = beta
...
fig.suptitle('Counts for features with SNR=%s, n=%s, p=%s varied over s' % (SNR[2], n_samples[2], n_features))
for i, ax in enumerate(axes.flatten()):
    ...
    ax.set(title=("s=%s, TPR=%s" % (sparsity[i], TPR[i])), xlabel="features", ylabel="counts")

fig.subplots_adjust(hspace=0.3)
plt.savefig("assignment-3/images/sparsity_90.png")
plt.show()