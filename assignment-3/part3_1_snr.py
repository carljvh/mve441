...
counts = np.zeros((len(SNR),n_features))
betas = np.zeros_like(counts)
for i, snr in enumerate(SNR):
    X, y, beta = simulate_data(n=n_samples[2], p=n_features, rng=np.random.default_rng(seed=42), sparsity=sparsity[1], SNR=snr)
    df = pd.DataFrame(X)
    df['y'] = y
    betas[i] = beta
...
fig.suptitle('Counts for features with n=%s, p=%s, s=%s varied over SNR.' % (n_samples[2], n_features, sparsity[1]))
for i, ax in enumerate(axes.flatten()):
    ...
    ax.set(title=("SNR=%s, TPR=%s" % (SNR[i], TPR[i])), xlabel="features", ylabel="counts")

plt.savefig("assignment-3/images/SNR_90.png")
plt.show()
